...
fn send_packet(port: &mut dyn serialport::SerialPort, payload: &[u8]) -> std::io::Result<Vec<u8>> {
...
fn enroll_fingerprint(port: &mut dyn serialport::SerialPort, id: u16) -> std::io::Result<()> {
...
fn verify_fingerprint(port: &mut dyn serialport::SerialPort) -> std::io::Result<()> {
...
fn list_fingerprints(port: &mut dyn serialport::SerialPort) -> std::io::Result<()> {
...
fn delete_fingerprint(port: &mut dyn serialport::SerialPort, id: u16) -> std::io::Result<()> {
...
fn main() {
    ...
    loop {
        println!("\nChoose mode: [1] Enroll  [2] Verify  [3] List  [4] Delete  [q] Quit");
        let mut input = String::new();
        std::io::stdin().read_line(&mut input).unwrap();

        match input.trim() {
            "1" => {
                println!("Enter ID (1-127): ");
                let mut id_str = String::new();
                std::io::stdin().read_line(&mut id_str).unwrap();
                let id = id_str.trim().parse::<u16>().unwrap_or(1);
                enroll_fingerprint(&mut *port, id).unwrap();
            }
            "2" => {
                verify_fingerprint(&mut *port).unwrap();
            }
            "3" => {
                list_fingerprints(&mut *port).unwrap();
            }
            "4" => {
                println!("Enter ID to delete (1-127): ");
                let mut id_str = String::new();
                std::io::stdin().read_line(&mut id_str).unwrap();
                let id = id_str.trim().parse::<u16>().unwrap_or(1);
                delete_fingerprint(&mut *port, id).unwrap();
            }
            "q" => {
                println!("Goodbye!");
                break;
